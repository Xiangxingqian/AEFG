...
import aefg.entry.AndroidConstants;
...
public class AndroidAnalysis {
	...
	private List<InvokeStmt> startActivities = Global.v().getStartActivityStmts();
	...
	public void analyze(){				
		...
		addToGlobal(appClasses);
		
		//1. get all the startActivity statements.
		parseStartActivity(appClasses);
		
		//2. generate <intent, startActivity> pair.
		genITS();
		System.out.println("ITS has been done! Size is: "+Global.v().getITSPairs().size());

		//3. replying on CallGraph, get entry method of startActivity's belonging method.
		EM.v().genEm();
	...
	private void parseStartActivity(List<SootClass> appClasses) {
		for(SootClass sootClass:appClasses){
			if(!sootClass.isInterface()){
				for(SootMethod sm:sootClass.getMethods()){
					if(sm.isConcrete()){
						extractStartActivityFrom(sm);
					}
				}
			}
		}
	...
	public void genITS(){
		for(InvokeStmt is:startActivities){
			...
			IntentFlowAnalysis ifa = new IntentFlowAnalysis(method);
			ifa.genITS(is);	
		}
	}
	
	private void addToGlobal(List<SootClass> applicationClasses){
		Global.v().getSootClasses().addAll(applicationClasses);
	}
...
//		TempGraph.v().getTempEdges().add(tempEdge);
	...
	/**
	 * extract startActivity statement in sootMethod.
	 * */
	private void extractStartActivityFrom(SootMethod sootMethod){
		...
		for(Unit unit:chain){			
			...
			if(unit instanceof InvokeStmt){ 
				...
				if(signature.equals(AndroidConstants.Sig.STARTACTIVITY)||
						signature.equals(AndroidConstants.Sig.STARTACTIVITYFORRESULT)||
						signature.equals(AndroidConstants.Sig.STARTACTIVITY_CONTENT)||
						signature.equals(AndroidConstants.Sig.STARTACTIVITYFORRESULT_CONTENT)){
